...
- **GitHub Dark тема**: Единая цветовая схема для zsh, tmux и Vim
...
### Vim
- ✅ GitHub Dark цветовая схема (официальная тема)
- ✅ Без номеров строк (чище для редактирования YAML)
- ✅ Блок-курсор в Normal/Visual режимах (хорошо виден)
- ✅ OSC 52 clipboard через SSH+tmux (копирование без номеров строк)
- ✅ Автоматическая установка vim-plug и плагинов
- ✅ Минимальное влияние на производительность (~10ms загрузка темы)
...
**Примечание**: curl требуется для автоматической установки vim-plug при первом запуске Vim.
...
### 5. Установка Vim плагинов
При первом запуске Vim автоматически:
1. Скачает vim-plug (plugin manager)
2. Установит GitHub Dark тему
3. Перезагрузит конфигурацию

```bash
vim
# Первый запуск займет ~5-10 секунд для установки плагинов
# При следующих запусках Vim загружается мгновенно
```

Если хотите установить плагины вручную:
```bash
